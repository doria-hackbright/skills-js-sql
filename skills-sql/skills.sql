...
SELECT m.name AS model, b.name AS brand, b.headquarters 
...
-- Inner joins take the "INtersection" between two tables, which means only rows where
    --both tables contain rows based on the fields the two tables are joined on.
-- On the flip side, left joins take all rows from the table left of the join statement
    --regardless if the other table has a corresponding row.
...
INSERT INTO models (year, brand_name, name) VALUES
(2015, 'IIHS Safety Award', (SELECT id 
                             FROM models 
                             WHERE name = 'Malibu' AND brand_name = 'Chevrolet';)),
(2015, 'IIHS Safety Award', (SELECT id 
                             FROM models 
                             WHERE name = 'Outback' AND brand_name = 'Subaru';)),
